...
public static class DiffPlaybackPlanner
{
	...
	public static PlanResult CreatePlan(FileAnalysis current, FileAnalysisDiff change, string file)
	{
		...
		List<DiffEdit> edits = change.Kind == FileAnalysisChangeKind.Modify
			? (change.Edits ?? new())
			: BuildEditsForAddOrDelete(oldLines, newLines);
		...
		while (iOld < oldLines.Count || iOp < edits.Count)
		{
			if (iOp < edits.Count)
			{
				...
				if (op.Kind == DiffOpType.Remove && iOld < oldLines.Count && op.Index == iOld)
				{
					...
					removed.Add(new BlockPlan { LineType = oldLines[iOld].Type, StartLength = startLen, EndLength = 0, IsNew = false });
					iOld++;
					iOp++;
					continue;
				}
				...
				{
					LineGroup g;
					if (op.Index >= 0 && op.Index < newLines.Count) g = newLines[op.Index];
					else g = new LineGroup { Type = op.LineType, Length = op.NewLength ?? 0 };
				...
				if (op.Kind == DiffOpType.Resize && op.Index == newSeq.Count && iOld < oldLines.Count)
				{
					...
					newSeq.Add(new BlockPlan { LineType = oldLines[iOld].Type, StartLength = startLen, EndLength = endLen, IsNew = false });
					iOld++;
					iOp++;
					continue;
				}
			...
			if (iOld < oldLines.Count)
			{
				...
				newSeq.Add(new BlockPlan { LineType = oldLines[iOld].Type, StartLength = len, EndLength = len, IsNew = false });
				iOld++;
				continue;
			}
